...
class PolimorfniSpremnik{
    Spremnik *p_lik;
    void Testiranje() const { if(!p_lik) throw std::logic_error("Nespecificiran spremnik");}
public:
    PolimorfniSpremnik() : p_lik(nullptr) {}
    ~PolimorfniSpremnik() { delete p_lik; }
    ...
    PolimorfniSpremnik(PolimorfniSpremnik &&lik) { p_lik = lik.p_lik; lik.p_lik = nullptr; }
    PolimorfniSpremnik &operator =(const PolimorfniSpremnik &lik){
        Spremnik *spremnik = nullptr;
        if(lik.p_lik != nullptr) spremnik = lik.p_lik->DajKopiju();
        delete p_lik;
        p_lik = spremnik;
        return *this;
    }
    ...
    double DajTezinu() const { Testiranje(); return p_lik->DajTezinu(); }
    double DajUkupnuTezinu() const { Testiranje(); return p_lik->DajUkupnuTezinu(); }
    void Ispisi() const { Testiranje(); p_lik->Ispisi(); }
};
