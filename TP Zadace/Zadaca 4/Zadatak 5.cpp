...
Laptop::Laptop(int ev_broj, std::string naziv, std::string karakteristike){
    ...
    Laptop::karakteristike = karakteristike; kod_koga_je = nullptr;
}
...
Administracija::Administracija(const Administracija &l){
    for(auto it = l.studenti.begin(); it != l.studenti.end(); it++){
        Student *stud = new Student((*it->second).DajIndeks(), (*it->second).DajGodinuStudija(), (*it->second).DajImePrezime(), (*it->second).DajAdresu(), (*it->second).DajTelefon());
        *stud = *it->second;
...
void Administracija::RegistrirajNovogStudenta(int indeks, std::string godina_studija, std::string ImePrezime, std::string Adresa, std::string BrojTelefona){
    ...
    Student *pok = new Student(indeks, godina_studija, ImePrezime, Adresa, BrojTelefona);
...
void Administracija::RegistrirajNoviLaptop(int ev_broj, std::string naziv, std::string karakteristike){
    ...
    Laptop *pok = new Laptop(ev_broj, naziv, karakteristike);
...
void Administracija::ZaduziLaptop(int indeks, int ev_lap){
    Student *nadjenStud = nullptr;
    Laptop *nadjenLap = nullptr;
    for(auto &par : studenti){
        if(par.first == indeks){
